...
> - `figshare / Zenodo / BioStudies` — 6 occurrence(s), a repository name
...
XX makes that floor expensive, because the phenotype invoked to explain
...
different mechanism: an SPECIES-1 persister assay gave different
...
The two boundaries are tested here on a published DRUG-A-tolerance
classification of 217 clinical XX isolates, which deposits
alongside each call the starting most probable number, the growth rate, the
DRUG-B susceptibility and the killing readings the call was built from
...
handed out deliberately. GROUP-7 and colleagues ran exactly that
...
are separate (GROUP-2 et al. 2016 (2)). Two secondary deposits bound how
...
GROUP-5 and colleagues report DRUG-E-clavulanate against
...
culturability, a described phenotype of XX:
...
every arm of the GROUP-8 regimen carries.
...
XX, axenic culture, an auxotrophic STRAIN-1 panel and two
...
GROUP-1 and colleagues report that tolerance goes with resistance status and
...
datasets, and a prospective SPECIES-1 experiment. We assembled 78
...
(7, 14). DRUG-C and DRUG-B at one and ten times the minimum
...
described it: GROUP-1 and colleagues (6, 13), GROUP-7 and colleagues
