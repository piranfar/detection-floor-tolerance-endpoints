...
The deposit never states its limit of quantification [[R5]], so the floor has
...
= 0.00056 [[R63]]). The remaining two of the 33 carry the MDR label and fall
...
thinner (Mann-Whitney p = 9.1 × 10⁻¹⁴ [[R18]]). Of the 84 resistant isolates
...
fitness-neutral [[R21]][[R22]]. Seven of the nine pretreatment covariates the
...
one (exact McNemar p = 3.0 × 10⁻⁸ against an unpaired p of 2 × 10⁻⁵ [[R19]]).
Eleven leave the floor and none joins it (p = 9.8 × 10⁻⁴), and every isolate
whose headroom changes gains (Wilcoxon signed-rank p = 1.3 × 10⁻³³ [[R20]]).
...
interval-censored data [[R29]] at the most sensitive plating, the tenth
...
near 5 × 10⁵ CFU/mL [[R31]]. If the protocol fixes the starting density, the
...
turbidity [[R32]], and cloudiness counts live cells, dead cells, debris and a
...
to 6.72 over the same twenty-four hours. A second deposit [[R36]][[R8]] gives
...
separate (Brauner et al. 2016 [[R2]]). Two secondary deposits bound how far
from separate the two could be, without proving they are separate, which is
weaker and more honest than a proof would be.

Apramycin against *M. tuberculosis* [[R37]][[R7]] shows what reading a plate
...
clones sharing one ancestor [[R38]][[R6]] returns ρ = +0.043, p = 0.63,
...
in a hollow-fibre system [[R36]][[R8]]. Their Methods state 100 µL plated with
...
on their way to a different question [[R57]]. Under the deepest regimens the
...
recovered from patients whose solid-medium cultures return nothing [[R65]].
Rifamycin action on ribonucleic acid (RNA) polymerase converts a starved
population to over 90 per cent differentially detectable in vitro [[R66]], and
...
curve of 0.90 on its final external validation [[R58]]. Its eight predictors
...
defined 99.9 per cent killing, at least ten colonies must remain [[R31]] (for
...
starting density (isoniazid needs KatG and active cell-wall synthesis [[R39]],
rifampicin needs transcription [[R40]]). A population that is not dividing
...
followed in patients is now being read as within-host evolution [[R55]]. A
...
of any question about tolerance classification [[R71]]. Suppose a tolerance
...
[[R21]][[R22]]. That gap is a finding in its own right and belongs in the next
...
drug varied less [[R23]]. That conclusion is theirs, and it anticipates part
...
volume stated beside them [[R59]]. That is better practice than anything else
...
`CFU_LOD`, empty in all 272 rows [[R60]]. Nor does a detection limit appear in
...
endpoint cut on a fraction of the starting count [[R69]]. The five fields
...
assayed under rifampicin [[R5]]. Each isolate carries a minimum inhibitory
...
[[R23]][[R4]]. Moxifloxacin and isoniazid were used at one and ten times
...
[[R34]]. The file holds 2,775 readings. The analysis set, after dropping
...
[[R61]][[R62]]. It was 0.008 mg/L, within the CLSI quality-control range
for this strain [[R62]]. The exposure was set at ten times it, 0.08 mg/L.
...
written for this assay [[R46]]. As a consistency check, 50 imputed
...
[[R48]]. Both procedures share the same distributional assumptions, so
...
Committee (OxTREC 030-07) [[R51]]. The isolates are deposited
...
[[R8]][[R36]]. That last deposit was held out of every fitting step. The
...
receipts are in a public repository [[R52]]. The receipts record the
