...
table [[R9]][[R10]], a fixed ladder of rungs, not a continuous scale. The
...
The deposit never states its limit of quantification [[R5]], so the floor
...
p = 0.00056 [[R63]]); the remaining two of the 33 carry the MDR label and
...
ten-fold thinner (Mann-Whitney p = 9.1 × 10⁻¹⁴ [[R18]]), and 22 of the 84
...
fitness-neutral [[R21]][[R22]]. Seven of the nine pretreatment covariates
...
of 2 × 10⁻⁵ [[R19]]), eleven leave the floor and none joins it (p = 9.8 ×
10⁻⁴), and every isolate whose headroom changes gains (Wilcoxon signed-rank
p = 1.3 × 10⁻³³ [[R20]]). The interquartile range of starting density falls
by 0.58 log10. <!--supp-->
...
one written protocol [[R23]][[R4]]. It does.
...
Fitted as interval-censored data [[R29]] at the most sensitive plating, the
...
puts the target near 5 × 10⁵ CFU/mL [[R31]]. If the protocol fixes the
...
turbidity [[R32]], and cloudiness counts live cells, dead cells, debris and
...
[[R33]]. Second, the standard fixes what goes into the flask, while what
...
second deposit [[R36]][[R8]] gives the same result against its own stated
...
are separate (Brauner et al. 2016 [[R2]]). Two secondary deposits bound how
...
Apramycin against *M. tuberculosis* [[R37]][[R7]] shows what reading a
...
coli* clones sharing one ancestor [[R38]][[R6]] returns ρ = +0.043, p =
...
*Escherichia coli* in a hollow-fibre system [[R36]][[R8]]. Their Methods
...
number, on their way to a different question [[R57]]. Under the deepest
...
bacilli dominate pre-treatment sputum and rise during chemotherapy [[R64]],
...
[[R65]], and the mechanism has been reproduced experimentally, rifamycin
action on ribonucleic acid (RNA) polymerase converting a starved
population to over 90 per
cent differentially detectable in vitro [[R66]], the same drug class every
...
characteristic curve of 0.90 on its final external validation [[R58]]. Its
...
colonies must remain [[R31]] (for a standard inoculum of 5 × 10⁵ per mL
...
allele [[R21]][[R22]]. That gap is a finding in its own right and belongs
...
of the drug varied less [[R23]]; that conclusion is theirs, and it
...
with treatment history in the same isolates [[R51]]. We do not dispute a
...
beside it [[R59]]. That is better practice than anything else in this
...
`CFU_LOD`, empty in all 272 rows [[R60]]; nor does a detection limit appear
...
duration endpoint cut on a fraction of the starting count [[R69]]. The five
...
isolate (Vijay et al. 2024; eLife 93243 supplementary file 2) [[R5]]. Of
...
Benjamini–Hochberg procedure [[R14]] at a false discovery rate of 5 per
cent.
