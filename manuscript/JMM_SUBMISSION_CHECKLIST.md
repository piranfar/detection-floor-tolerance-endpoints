...
| Preferred reviewers | Elsevier asks for suggestions at submission. | **Three named, with a fourth in reserve, in `PREFERRED_REVIEWERS.md`:** Marcus A. Horwitz (UCLA), Tehmina Mustafa (Bergen) and Norbert O. Ndjeka (South African National Department of Health), with Joanne Turner (Nationwide Children's) as the alternate. Chosen under the author's rule — active in TB, no publication of their own on this paper's method — from 29 candidates, of whom 9 were verified in full and 5 disqualified, two of those by a pass whose only brief was to disqualify them. Every email comes from a printed correspondence line or an official institutional page, named in the file. |
| Highlights | Required at submission: 3 to 5 bullet points, each at most 85 characters including spaces, as a separate file with "highlights" in the file name. | Five bullets in `manuscript/highlights.md`, longest 76 characters, each naming a number that regenerates from the tables. Copied into the package as `05_highlights.md`. |
| Generative AI disclosure | Required where generative AI assisted the writing, in a new section titled exactly *Declaration of generative AI and AI-assisted technologies in the manuscript preparation process* per the journal's Guide for Authors. | Section titled to match, and it says what happened: Claude was used under the author's direction to read the manuscript and assist in running the Python analysis pipeline, and Claude Sonnet 5 was used to document and organise the analysis code and prepare its commits to the repository; no figure or image was AI-generated; no AI tool is an author; the author verified every quantity against the deposits and the pipeline and takes responsibility. In Materials and Methods, repeated in the cover letter. |
